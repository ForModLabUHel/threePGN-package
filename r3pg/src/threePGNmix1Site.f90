...
! Declare input variables
    ...
    integer :: nvariables, nMonths, noOfLayers, totThinning, vars(nvariables)
    real(kind=8), dimension(totThinning, 6) :: thinningInputs
    real(kind=8), dimension(noOfLayers, 17) ::  siteData
    ...
    real(kind=8), dimension(noOfLayers) :: Lat_site, StemNo_site, ASW_site, MinASW_site, & 
        ...
        SoilClass_site, endAge_site !(noOfLayers)
    integer, dimension(noOfLayers) :: nThinning_site, startMonth_site
...
!   Add thinning if present
	...
	do ii = 1, nmonths
		Tn(ii) = weather(ii,2)
		Tx(ii) = weather(ii,1)
		SolarRad(ii) = weather(ii,4)
		Rain(ii) = weather(ii,3)
		FrostDays(ii) = weather(ii,5)
...
!   Initiate the loop over sites
    do ii = 1, nmonths
		...
		do layer = 1, noOfLayers
			...
			Yr_C = Yr_C_i	!#!# soil model, need to think about this
			Yl_C = Yl_C_i	!#!# soil model, need to think about this
			O_C = O_C_i		!#!# soil model, need to think about this
			Yr_N = Yr_N_i	!#!# soil model, need to think about this
			Yl_N = Yl_N_i	!#!# soil model, need to think about this
			O_N = O_N_i		!#!# soil model, need to think about this
